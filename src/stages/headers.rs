    ...
    E: EnvironmentKind,
{
        ...
        'db: 'tx,
    {
        ...
        let (headers, reached_tip) = match self.consensus.fork_choice_mode() {
            ForkChoiceMode::External(mut chain_tip_watch) => {
                ...
                let prev_progress_block = txn
                    ...
                    .ok_or_else(|| {
                        StageError::Internal(format_err!("no header for block #{prev_progress}"))
                    })?;
                    ...
                    .await
                {
                    LinearDownloadResult::Done(buffered_headers) => (
                        Box::new(buffered_headers.into_values())
                            as Box<dyn Iterator<Item = (H256, BlockHeader)>>,
                        true,
                    ),
            ...
            ForkChoiceMode::Difficulty(fork_choice_graph) => {
                ...
                while headers.len() < headers_cap {
                    ...
                    if let Some(mut downloaded) = self
                        .download_headers(fork_choice_graph.clone(), starting_block, target_block)
                        .await?
                    {
                ...
                (
                    Box::new(headers.into_iter()) as Box<dyn Iterator<Item = (H256, BlockHeader)>>,
                    reached_tip,
                )
            }
        };
